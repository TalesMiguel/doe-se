<template>
  <div class="center" align="center">
    <v-card elevation="10" color="#F6E2E2" outlined width="1400" height="650">
      <v-form v-model="valid" ref="formInst" lazy-validation
        ><v-row>
          <v-col>
            <v-text-field
              v-model="cnpj"
        ...
        <v-row>
          <v-col>
            <v-text-field v-model="rua" :rules="ruaRules" label="Rua" required>
              ></v-text-field
            >
          </v-col>
          <v-col cols="1">
        ...
        <v-row>
          ...
          <v-col>
            <v-text-field v-model="cep" :rules="cepRules" label="CEP" required>
              ></v-text-field
            >
          </v-col>
        </v-row>
...
export default {
  ...
  data: () => ({
    ...
    nomeRepresentanteRules: [
      ...
      (v) =>
        (!/[^a-zA-Z\s]+/.test(v) && !/\s\s/.test(v) && v[0] !== ' ') ||
        'Nome invalido'
    ],
    tipoInst: null,
    tiposInst: ['ONG', 'Instituição religiosa', 'Instituição filantrópica'],
    rua: '',
    ruaRules: [(v) => !!v || 'Rua obrigatória'],
    numero: '',
    numeroRules: [
    ...
    bairro: '',
    bairroRules: [(v) => !!v || 'Bairro obrigatório'],
    cidade: '',
    cidadeRules: [(v) => !!v || 'Cidade obrigatória'],
    uf: null,
    ufs: ['SP', 'RJ', 'MG', 'ES'],
    cep: '',
    cepRules: [
    ...
    subTipos: ['n/a'],
    nomeInst: '',
    nomeInstRules: [(v) => !!v || 'Nome obirgatório'],
    nomeFantasia: '',
    telefoneRep: '',
    telefoneRepRules: [
      (v) => !!v || 'Telefone obrigatório',
      (v) =>
        (/(11|12|21)[0-9]{8}/.test(v) && v.length === 10) ||
        (/(11|12|21)9[0-9]{8}/.test(v) && v.length === 11) ||
        'Telefone invalido'
    ],
    emailInst: '',
    emailInstRules: [
